...
- 内存与磁盘缓存，设置中可查看并清除缓存。
- 预览显示且前台为受支持表格时，可用 `Space` 展开/收起、`Esc` 临时隐藏、`⌥C` 复制图片（成功后显示约 1 秒反馈）、`⌥O` 打开来源、`⌥R` 在 Finder 中显示本地图片、`⌥P` 固定/取消固定预览。
- 可设置图片所在列、开机自启、缓存上限和缓存保留时间。

> 快捷键只会在有效预览上下文中临时注册；ImagePeek 不监听所有普通按键，也不需要输入监控权限。
...
3. 在 WPS、Excel 或支持的飞书 Sheets 页面中选择包含完整图片 URL 的单元格。
